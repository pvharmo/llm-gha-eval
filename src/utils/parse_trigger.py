...
def parse_push_event(event_config):
    ...
    if "branches" in event_config:
        ...
        elif isinstance(event_config["branches"], list):
            print(event_config["branches"])
            conditions += f"to branches {format_list(event_config['branches'])}"
        else:
            conditions += f"to branches {event_config['branches']}"
    if "tags" in event_config:
        if "branches" in event_config:
            conditions += " and "
        if event_config["tags"] is None:
            conditions += "to any tags"
        elif isinstance(event_config["tags"], list):
            conditions += f"to tags {format_list(event_config['tags'])}"
        else:
            conditions += f"to tags {event_config['tags']}"
    ...
    conditions = parse_path_conditions(event_config, "branches" in event_config or "tags" in event_config or "branches-ignore" in event_config or "tags-ignore" in event_config)
    ...
    if conditions:
        return f" The workflow would run on a push event {conditions}."
    else:
...
def parse_pull_request_event(event_config):
    ...
    if conditions == "" and precondition != "":
        return f" The workflow would run {precondition} pull request {conditions}"
    elif conditions != "" and precondition != "":
        return f" The workflow would run {precondition} {conditions}"
    elif conditions != "" and precondition == "":
        return f" The workflow would run on pull request {conditions}"
    else:
...
def parse_schedule_event(event_config):
    if isinstance(event_config, list):
        ...
        raise ValueError("Invalid schedule event format.")
...
def parse_path_conditions(event_config, also: bool):
    ...
    if "paths-ignore" in event_config:
        if "paths" in event_config:
            ...
            path_conditions.append(f" with changes changes that are made to any files except those matching {format_list(event_config['paths-ignore'])}")
